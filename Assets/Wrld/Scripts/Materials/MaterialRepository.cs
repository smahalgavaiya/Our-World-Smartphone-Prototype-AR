...
namespace Wrld.Materials
{
    public class MaterialRepository
    {
                ...
                if (material == null)
                {
                    ...
                    else
                    {
                        if (IsHighlightMaterialName(materialName))
                        {
                            ...
                            {
                                Debug.LogFormat("Highlight material {0} has not been created with the appropriate color. Creating default material.",DLogType.Warning, materialName);
                            }
        ...
        public void ReleaseMaterial(string materialName)
        {
            ...
            {
                Debug.LogFormat("material {0} was not present",DLogType.Warning, materialName);
            }
        }
        ...
        public void ReleaseHighlightMaterial(string materialName)
        {
            ...
            {
                Debug.LogFormat("{0} : is not a highlight material. Names must begin with Highight",DLogType.Warning, materialName);
            }
        }
        ...
        public void CreateAndAddHighlightMaterial(string materialName, Color color)
        {
            if (IsHighlightMaterialName(materialName))
            {
                ...
                if (m_materials.TryGetValue(materialName, out record))
                {
                    Debug.LogFormat("material {0} already exists",DLogType.Warning, materialName);
                }
                else
                {
            ...
            {
                Debug.LogFormat("{0} : highlight material names must begin with Highight",DLogType.Warning, materialName);
            }
        }
