...
public class MainMenuManager : MonoBehaviour 
{
	...
	public void PreferredTitleScreen(int screenNumber)
	{
		...
		{
			if (PlayerPrefs.GetInt("AutoSwitchToggle", 0) == 0 ? false : true == true) SetNewCoroutine(ref _autoSwitchTitleScreen, ChangeTitleScreen());
			else if (_autoSwitchTitleScreen != null) StopCoroutine(_autoSwitchTitleScreen);
	...
	{
		if (value) SetNewCoroutine(ref _autoSwitchTitleScreen, ChangeTitleScreen());
		if (!value) if (_autoSwitchTitleScreen != null) StopCoroutine(_autoSwitchTitleScreen);
